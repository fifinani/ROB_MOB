...
/////////////////////////////////////////////////////
    ...
    //    tree1.affiche_vect(v);
        ...
        //    std::cout << "new point ="<< new_point << '\n';
            ...
            if( node_added.get_distance(node2)<q  ){
                reached=true;
                std::cout << "REACH" << '\n';
...
//              ROS
    ...
    for (int i = 0; i < taille; i++) {
        point.x=tree1.getNodeAt(indices[i]).getPoint().x;
        point.y=tree1.getNodeAt(indices[i]).getPoint().y;

        nh_.advertise<geometry_msgs::Point>("/position", taille );
